...
df_lol %>%
  ...
  # deler på totalt for alle næringerog ganger med 100 for å få prosent
...
df_oppg3_wide %>% 
 filter(næring != c('Fastlands-Norge', 'Markedsrettet virksomhet', 'Ikke-markedsrettet virksomhet')) %>%  
  filter(year >= 2012) %>% 
  ggplot(aes(x=year, y=faste, group = næring)) + 
  geom_col(col="blue", fill="blue") +
  geom_col(aes(x=year, y=løpende, group = næring), fill="red")+
  facet_wrap(~næring)
