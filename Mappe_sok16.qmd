...
df_oppg3_wide %>% 
  ...
  geom_col(data = filter(df_oppg3_wide, pris == "Løpende"), aes(y = verdi)) +
  geom_col(data = filter(df_oppg3_wide, pris == "Faste"), aes(y = verdi)) +
  facet_wrap(~pris)+
  scale_fill_manual(values=c('lightcoral', 'cornflowerblue')) +
  labs(x='År', y='Millioner kroner', fill='pris', title = 'Utvinning av råolje og naturgass, inkl. tjenester') +
  theme_minimal(base_size=9)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
