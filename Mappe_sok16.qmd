...
df_graf_1 <- df2 %>% 
 ...
 #          'Offentlig administrasjon og forsvar'))
  ggplot(aes(x = verdi, y = næring, color = næring, group = næring))+
  geom_line()+
...
#df_graf_2 <- df2 %>% 
 ...
 # ggplot(aes(x = year, y = log(verdi), color = næring, group = næring))+
 # geom_line()+
 # theme(legend.key.size = unit(0.1, 'cm'))+
 # scale_x_discrete(guide = guide_axis(n.dodge = 2))
```
...
df_oppg3_wide %>% 
 filter(næring %in% c('Fastlands-Norge', 'Markedsrettet virksomhet', 'Ikke-markedsrettet virksomhet')) %>%  
  filter(year >= 2012) %>% 
  ggplot(aes(x=year, group = næring, position = 'dodge')) + 
  geom_col(aes(y = faste)) +
  geom_col(aes(x=year, y=løpende))+
  facet_wrap(~næring)+
...
```{r}
df2 %>% 
  filter(var %in% c('Bruttoprodukt i basisverdi. Faste 2020-priser (mill. kr)', 'Bruttoprodukt i basisverdi. Løpende priser (mill. kr)')) %>% 
